...
Feature 1 -- liquid_hydrocarbon  (weight 0.25, prior_mean 0.02)
  ...
  Alternative: Hayes (2016) Ann.Rev. gives total lake/sea area ~0.65x10^6 km^2
               = 0.013 of 8.3x10^7 km^2 surface = 1.3%.  Alternative prior: 0.015.
               This is a minor revision; 0.02 is acceptable.

Feature 2 -- organic_abundance  (weight 0.20, prior_mean 0.60)
  Source: Barnes et al. (2007), Icarus 186:242; Le Mouelic et al. (2019),
          Icarus 319:121.
...
Feature 3 -- acetylene_energy  (weight 0.20, prior_mean 0.30)
  ...
  Basis:  H2 downward flux 10^2^5 mol/s (Strobel 2010) -- this is OBSERVED and
...
Feature 4 -- methane_cycle  (weight 0.15, prior_mean 0.40)
  ...
  Alternative: Birch et al. (2025) Titan After C-H Ch.10 notes cycle is
               concentrated at 45-60 degN/S; a latitude-weighted global mean
...
Feature 8 -- subsurface_ocean  (weight 0.02, prior_mean 0.10)
  ...
  Basis:  k2 = 0.589 +/- 0.150 confirms global ocean.  Affholder (2025) shows
...
Scientific basis:
  - Higher impactor flux during LHB created more and larger impact melt oases.
    ...
    water for 10^3-10^4 yr.  Earlier Titan had more of these.
...
Assumption required from user:
  WARNING WHICH epoch? "Past" spans 4.5 Gya (accretion) to ~500 Mya (recent).
...
Limitations and assumptions for FUTURE mode:
  WARNING No Cassini dataset directly constrains future habitability.
  ...
  WARNING The red giant scenario (Lorenz 1997) assumes:
  ...
  WARNING The "several hundred Myr" window is highly uncertain:
...
PAST -- missing datasets:
  ...
  3. Titan internal heat flow model output
     ...
     WARNING ASSUMPTION: Internal heat flow is not spatially mapped. Use scalar.

PRESENT -- missing datasets:
  4. Atmospheric H2 depletion map (spatially resolved)
     -> Strobel (2010) gives a global flux; no spatially resolved map exists.
     Source: CIRS/INMS data; no spatial map published
     WARNING Applied as global constant in feature 3 (acetylene_energy).
     Clarification: The acetylene_energy feature uses SAR backscatter as a
     SURFACE proxy for organic depletion, not the atmospheric H2 flux directly.
     This is an ASSUMPTION that should be acknowledged.

  5. RADAR emissivity (passive radiometry) for subsurface properties
     -> Cassini RADAR radiometer data gives emissivity proportional to subsurface composition
...
FUTURE -- missing datasets:
  ...
  7. Titan atmospheric escape model
     ...
     WARNING ASSUMPTION: Atmosphere persists. This is theoretically supported but
...
D2: Near-future solar-warming window -- ACCEPTED: 100-400 Myr, uniform warming
    ...
    WARNING  EXPLICIT ASSUMPTION -- uniform global warming:
...
Le Mouelic et al. (2019)      Icarus 319:121     doi:10.1016/j.icarus.2018.09.017
...
FUTURE_FEATURES: List[str] = [
    ...
    "dissolved_energy",          # REPLACES acetylene_energy (C2H2 in warm water)
    ...
    if mode == TemporalMode.PRESENT:
        return TemporalPriorSet(
            ...
            weights=(
                ...
                0.20,  # acetylene_energy        -- confirmed H2 depletion (Strobel 2010)
            ...
            prior_means=(
                ...
                0.350,  # acetylene_energy:   H2 depletion confirmed (Strobel 2010)
        ...
        return TemporalPriorSet(
            ...
            prior_means=(
                0.100,  # liquid_hydrocarbon: lakes beginning to form.
                         # Tobie (2006): methane outgassing began ~500 Mya-1 Gya.
                         # At 1.0 Gya lakes are sparse; ~10% of present lake area.
...
def describe_prior_changes(
    ...
    if mode == TemporalMode.PRESENT:
        ...
        return (
            ...
            "  [WARNING]  ASSUMPTION: All future features derived from CURRENT data.\n"
            "  [WARNING]  ASSUMPTION: Red giant window ~6 Gya; 'several hundred Myr'.\n"
            "  [WARNING]  No Cassini dataset directly constrains future state."
        )
