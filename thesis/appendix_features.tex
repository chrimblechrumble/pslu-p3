...
\begin{figure}[H]
  ...
  \caption{All eight habitability-proxy feature maps at the present (Cassini)
    ...
    and prior mean $\mu_i$ are shown for each panel.  In the Cassini SAR
    coverage gaps ($\sim$33\% of the globe), the SAR-primary features
    ($f_1$, $f_3$, $f_8$) lose direct coverage; there the pipeline substitutes
    the geomorphological ($f_7$) and topographic ($f_6$) proxies, or assigns
    the feature prior mean $\mu_i$ where no proxy is available
    (Section~\ref{sec:nan}).  Such substituted pixels contribute no evidence
    rather than incorrect evidence, so no panel is left blank.  The vertical discontinuity
...
The primary source is the \citet{Lopes2019} lake polygon class (terrain
...
to organic material.  Confirmed empty lake basins from the \citet{BirchHayesSAR}
...
as 69--78\,kJ\,(mol\,C)$^{-1}$ (equivalently per mol CH$_4$), roughly seven times the $\sim$$-10.6$\,kJ\,(mol\,CH$_4$)$^{-1}$ apparent minimum free energy that sustains terrestrial methanogens \citep{Hoehler2001}.  Acetylene is produced continuously by UV photolysis of methane
...
\paragraph{Implementation.}
The methane cycle intensity is a weighted composite of up to three
independently computed components; where a component's source layer is
unavailable, its weight is redistributed proportionally among those present:
\begin{enumerate}
  \item \textbf{Meridional temperature gradient} $|\partial T/\partial\phi|$
    (weight 0.50): the magnitude of the latitudinal gradient of the CIRS
    surface-temperature field (Eq.~\ref{eq:jennings}).  Regions where the
    surface temperature gradient is steepest experience the strongest
    differential evaporation and precipitation, driving the most vigorous
    methane transport.  This component is included only where the temperature
    field carries meaningful spatial variance.
  \item \textbf{Fluvial channel density} (weight 0.35): the
    \citet{Miller2021channels} channel network, rasterised and smoothed over
    $\sim$50\,km (an 11-pixel box filter), a direct record of where methane
    runoff and erosion occur.
  \item \textbf{Latitude prior} (weight 0.15): the function
    $\exp[-(|\phi| - 50\textdegree)^2/(25\textdegree)^2]$, a Gaussian centred
    on $\pm 50\textdegree$ following the general-circulation-modelling result
    that the storm-track latitudes host the primary methane-cycle activity
    during the Cassini epoch \citep{MitchellLora2016}.
\end{enumerate}

The latitude prior is always available, so this feature has 100\% global
coverage by construction and requires no backfill.
...
\paragraph{Implementation.}
A weighted sum of four spatial components:
\begin{equation}
  f_5 = 0.25\,\hat{s} + 0.35\,\hat{m} + 0.15\,\hat{p} + 0.25\,\hat{c}
  \label{eq:feat5}
\end{equation}
where $\hat{s}$ is the normalised topographic slope $|\nabla z|$ from the
GTDE DEM (high slopes drive physical transport and mixing); $\hat{m}$ is a
lake-margin mask formed by a 3-pixel ($\sim$13\,km) dilation of the
confirmed-liquid (Birch+2017) shoreline, combined with labyrinth terrain
(weighted 0.7); $\hat{p}$ is a paleo-lake indicator marking Birch+2017 empty
basins (former shorelines); and $\hat{c}$ is the Gaussian-smoothed ($\sigma = 3$ pixels)
channel density from the \citet{Miller2021channels} dataset, normalised at the
2nd--98th percentile range.  The lake-margin component receives the highest
within-feature weight (35\%) because shorelines concentrate all three relevant
...
\paragraph{Scientific basis.}
Local topographic roughness at $\sim$50\,km scale correlates with the
...
\paragraph{Implementation.}
Local standard deviation of GTDE elevation computed in an $11\times11$ pixel
(radius 5, $\sim$49\,km) sliding window, normalised to $[0,1]$ at the 2nd--98th
percentile range.  South polar gaps in the primary GTDE are filled with the
\citet{Corlies2017Titan} globally interpolated DEM prior to roughness computation.
...
of radius 7 pixels ($\sim$31\,km), normalised by the theoretical maximum
...
annular structures in the Cassini RADAR mosaic, interpreted as ancient
impact-melt rims or cryolava flow fronts \citep{Wood2010}.  Rather than
placing kernels at catalogued craters, the annuli are detected
morphologically: each pixel's mean backscatter in a small (5-pixel) window
is compared with its mean in a larger (25-pixel, $\sim$110\,km) window, and
pixels forming a bright ring around a relatively dark interior (small-window
mean exceeding the large-window mean, with the surroundings below the global
median) receive a positive annular score.  This first-order ring detector
captures the qualitative annular signature without geometric circle fitting;
it has not been validated against the \citet{Hedgepeth2020} crater catalogue,
so the enhancement is deliberately conservative.

The annular score is normalised to $[0,1]$ and added to the floor with a
maximum boost of $+0.30$, the sum clipped to $[0, 1]$.  Because the floor is
low ($\mu_8 = 0.03$) and only a small number of pixels register a strong
annular signature, the spatial variance of $f_8$ is very low: it is 0.03
almost everywhere, rising towards $\sim$0.33 only at the strongest annular
structures.
