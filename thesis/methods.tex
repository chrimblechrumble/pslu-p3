...
\subsection{Classifier Training and Feature Importances}
\label{sec:classifier}

The habitability maps presented in Chapter~\ref{ch:results} are produced by
the sklearn Random Forest (RF) pipeline, which uses the Bayesian posterior
as input to train a classifier rather than returning the Bayesian posterior
directly.  The RF pipeline produces output values that are not bounded by
$P_\text{max} = 0.696$ (Eq.~\ref{eq:bounds}); a polar lake shore at
$P(H) = 0.85$ is an RF probability, not a Beta posterior mean.
The Bayesian formula gives the physically interpretable prior-posterior
quantities described in Sections~\ref{sec:prior}--\ref{sec:bounds}; the RF
provides data-driven spatial discrimination and feature importances.

Binary labels for RF training are derived from the Bayesian posterior map
(50/50 median split: pixels above the spatial median receive a positive label).
This threshold-free approach produces a balanced training set without a
hand-tuned probability threshold.  Permutation feature importances
\citep{Breiman2001} are computed after fitting: each feature layer is
randomly permuted and the accuracy decrease is recorded as that feature's
importance.  The ratio of data-derived importance to prior weight $w_i$ is
the primary validation metric for the prior specification
(Section~\ref{sec:importances}).
...
computed by the full sklearn RF pipeline independently of the animation
and are used for all quantitative comparisons in this chapter.
