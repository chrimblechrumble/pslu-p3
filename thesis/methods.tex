...
\begin{enumerate}
    \item A one-off data-acquisition stage to gather relevant datasets from varied raw and derived sources. 
    \item A data preprocessing stage: e.g. some map data needs converting into a canonical mapping projection so that different features can be computed/extracted from the same Titan location  
    \item Extraction of eight habitability-proxy features from the data
    \item Bayesian conjugate updating to produce a posterior habitability probability map at each of five named geological 'anchor' epochs, (\textit{Past}, \textit{Lake Formation}, \textit{Present}, \textit{Near Future}, \textit{Future})
    \item Temporal scaling of features between anchor epochs to produce 72 geological epochs from $-3.8\gya$ to $+6.5\gya$ 
    \item Ranking of named surface sites by posterior probability
\end{enumerate}

Acronyms, notation, and the full dataset catalogue are defined in Appendix~\ref{app:notation}.
