...
\begin{enumerate}
  \item \textbf{VIMS observation density} (50\%): the number of VIMS
    ...
    cycling zones \citep{Le-Mouelic2019}.
...
A random forest classifier is also trained on binary labels derived from the
posterior map (50/50 median split: pixels above the spatial median receive a
positive label).  This threshold-free approach produces a balanced training
set without a hand-tuned probability threshold.  Permutation feature
importances \citep{Breiman2001} are computed after fitting: each feature
layer is randomly permuted (breaking its spatial correlation with the labels)
and the accuracy decrease is recorded as that feature's importance.  The
ratio of data-derived importance to prior weight $w_i$ is the primary
validation metric for the prior specification
...
2.5\% above the present value.  At this level of perturbation, the surface
...
identical to the PRESENT result), and as the upper anchor for the PCHIP
interpolation in the multi-epoch reconstruction.
...
$t \approx +5.1\gya$ and last at $+5.9\gya$, at which point $T_s = 466$\,K.
At $+6.0\gya$ the luminosity collapses to $0.8\,L_\odot$ (white dwarf), and
the surface temperature drops to $\sim$89\,K.
...
reconstruction.  The approach taken here uses the five epoch-specific
posterior maps as anchor states and generates intermediate maps through
pixel-wise interpolation.

The interpolation approach is the full-inference method, in which
independently derived epoch-specific Bayesian posteriors --- each reflecting
the full observational evidence at that epoch, with appropriate prior
specifications and feature sets --- are used as the anchor states.  This
method uses the data-driven spatial structure of each independent posterior,
incorporating the full feature-set information at each anchor epoch,
rather than simply scaling the present-day maps by temporal factors.

Intermediate maps at epochs between the anchors are generated using
Piecewise Cubic Hermite Interpolating Polynomial (PCHIP) interpolation
applied pixel-by-pixel across the 1802$\times$3603 grid.  PCHIP was chosen
over standard cubic splines because it preserves monotonicity within each
interpolation interval, preventing oscillatory behaviour that could produce
physically implausible intermediate values.  Interpolation is applied between
the LAKE\_FORMATION, PRESENT, and NEAR\_FUTURE anchors for epochs spanning
$-1.0$ to $+0.25\gya$.

For epochs earlier than $-1.5\gya$, a transition blending scheme is employed
rather than directly using the PAST anchor posterior in the interpolation.
This is necessitated by the past-epoch Cassini feature contamination documented
in Appendix~\ref{app:assumptions} (A4): the PAST posterior unrealistically
inflates north polar scores due to the present-era SAR lake signature being
present even at $-3.5\gya$.  Instead, a rescaled version of the analytically
computed Bayesian posterior, adjusted to match the probability scale of the
data-driven posteriors, is used for the pre-lake-formation frames, with a
five-epoch linear transition blend between this Bayesian output and the PCHIP
interpolation at the $-1.5$ to $-1.0\gya$ boundary.  For epochs from
$+0.25$ to $+4.0\gya$, the map is held constant at the NEAR\_FUTURE posterior,
reflecting the minimal physical change in this stable period.  A linear blend
from the NEAR\_FUTURE to the FUTURE anchor over $+4.0$ to $+5.9\gya$ captures
the gradual warming-driven transition from the hydrocarbon lake regime to the
global ocean state.
