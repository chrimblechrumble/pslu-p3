...
1. Jennings et al. (2019) ApJL 877, L8
   ...
   Surface brightness temperatures 90°S–90°N in 10° bins,
...
2. Schinder et al. (2011) Icarus 215, 460–474
   ...
   Mid-southern latitudes (31–53°S), 0–300 km altitude.
...
def jennings_surface_temperature(
    ...
    covering 90°S–90°N in 10° latitude bins.

    The formula is valid over:
      * Latitude  L  ∈  [−90, +90]  degrees
      * Year      Y  ∈  [−4.9, +8.1]  from equinox  (2004 Oct – 2017 Sep)
    ...
    ----------
    lat_deg:
        Latitude in degrees, south negative (−90 to +90).
    year_ce:
    ...
    latitude in degrees.  The cosine argument is in radians because the
    ...
    * Amplitude  (93.53 − 0.095·Y): peak temperature declining ~1 K over
      the mission as the warmest latitude shifts from 13°S to 24°N.
    * Phase  (L + 0.85 − 3.2·Y): the cosine peak latitude, which tracks
      the subsolar point northward at ~3.2°/yr relative to equinox.
    * Width  (0.0029 − 0.00006·Y): flattening of the pole-to-equator
    ...
    At Ls=313° (2005): peak 93.97 K near 13°S.   (paper: 93.9 K at 13°S)
    At Ls=90°  (2017): peak 92.83 K near 24°N.   (paper: 92.8 K at 24°N)
    Mission-wide range: 88.7–94.0 K.
    Standard deviation of fit: 0.4 K; worst period (Ls ≈ 26°): 0.7 K.
    ...
    phase_deg = L + 0.85 - 3.2 * Y         # latitude-offset in degrees
...
def jennings_temperature_map(
    ...
    ----------
    lats_deg:
        1-D (or any-shape) array of latitudes in degrees (−90 to +90).
    year_ce:
        Calendar year.
    ...
    phase_deg = lats + 0.85 - 3.2 * Y          # degrees offset per element
...
def jennings_temperature_grid(
    ...
    ----------
    lat_grid:
        2-D float32 array of latitudes in degrees, shape (nrows, ncols).
    year_ce:
        Calendar year.
...
#:   "latitude"    : float (degrees south, positive = south)
...
# ===========================================================================
#
# Simplified near-surface T(z) from Huygens HASI at 10°S.
...
HASI_NEAR_SURFACE = {
    "latitude":     -10.0,  # °S (actually 10°S south, positive = south)
